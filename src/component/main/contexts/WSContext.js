...
const WSContextProvider = ({ children }) => {
  ...
  const connect = async () => {
    disconnect();
    const url = `${process.env.REACT_APP_WS_URL}videoimports/`;
  ...
  const connectIMG = async () => {
    disconnect();
    const url = `${process.env.REACT_APP_WS_URL}multipleimagesimport/`;
    ws.current = new WebSocket(url);
    if (ws.current) {
      ws.current.onopen = () => {
        //  alert("Đã kết nối websocket");
  ...
  // Context data
  const wsContextData = { ws, connect, disconnect, connectIMG };
